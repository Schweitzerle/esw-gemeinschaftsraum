...
- [x] Feedback-Runde 8 (22.07.2026): (a) Login-Komfort — rollierende Session (`shouldRenewSession`, ab 30 Tagen Alter erneuert `hooks.server.ts` das Cookie auf volle 180 Tage; Cookie-Optionen zentral in `issueSessionCookie`), damit aktive Geräte das Haus-Passwort nie wieder eingeben müssen; (b) Auge-Schalter auf der Login-Seite (`aria-pressed`, nur nach Hydration gerendert, No-JS bleibt sauber). Bewusst NICHT gebaut: Passwort clientseitig speichern/vorbefüllen — das httpOnly-Cookie ist strikt sicherer. 62 Unit, 37 E2E grün; Verlängerung gegen `node build` mit selbst signiertem 40-Tage-Cookie verifiziert (Set-Cookie kommt, bei 2 Tagen nicht)
- [x] Nix-Packaging des Hosters gemergt (PR #5): flake + NixOS-Modul (systemd-Hardening, Secrets per LoadCredential), `origin = null` schaltet automatisch auf PROTOCOL_HEADER/HOST_HEADER. PR #6 (Impressum) hängt an Prettier (`<br>` statt `<br />`) — kommentiert.
